...
public class CustomerView {
    ...
    // customers本身 不能被重新赋值，即 final 限制了对引用的改变，不能将 customers 指向另一个 CustomerList 对象 即之后执行这种customers = new CustomerList(20);都是非法的
    // CustomerList 对象的内部状态可以被修改，你可以向列表中添加、删除或修改客户
    public CustomerView() {
        Customer cust = new Customer("张三",  30, '男', "010-56253825",
                "abc@email.com");
    ...
    private void modifyCustomer() {
        ...
        cust = new Customer(name, age, gender, phone, email);
    ...
    private void deleteCustomer() {
        ...
        Customer cust = null;
        for (;;) {
            System.out.print("请选择待删除客户编号(非0； -1退出)：");
            index = CMUtility.readInt();
            if (index == -1) {
                return;
            }
        ...
        boolean flag = customers.removeCustomer(index - 1);
        if (flag) {
            System.out
                    .println("---------------------删除完成---------------------");
        } else {
